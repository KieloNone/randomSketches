...
//Shadow mapping based on https://forum.processing.org/two/discussion/12775/simple-shadow-mapping
...
void setup(){
  ...
  // FOV for rendering from spotlight view
  lightFOV=PI*0.35;
}

void draw()
{
  //moonlander init, and position for spotlight
  //moonlander.update();
  //float moon = moonlander.getIntValue("track1");
  //lightPos.x= moon;lightPos.y=40.0;lightPos.z=0.0;
  
  //position for spotlight, varies in time
  lightPos.x= 400*(sin(t*0.01)+1.0);lightPos.y=40.0;lightPos.z=0.0;

  //render scene from spotlight view
  shadowMap.beginDraw();
 ...
 shader(defaultShader);
  ...
  // render white blob, where the spotlight is
  shader(lightShader);
  pushMatrix();
    fill(255);
...
void renderLandscape(PGraphics canvas,boolean defaultPass)
{
  ...
  // defaultpass=false for rendering the shadowbuffers, defaultpass=true for the final scene
  if(defaultPass)
  {
    //
    float cameraZ = (height/2.0) / tan(lightFOV/2.0);
    ...
    //set where the scene is viewed (either real scene, or in the perspective of either of the lights)
    if((cameraView==1)) {
...
//the following code relies heavily on the tutorial from internet
void updateDefaultShader() {
   ...
   PMatrix3D shadowTransform2 = new PMatrix3D(
    ...
    //EM: is not strictly needed if you write your own vertex shader 
